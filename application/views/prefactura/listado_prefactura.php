...
<section class="content">   
    <div class="box box-primary">
        <div class="box-header with-border">
            <h3 class="box-title"><i class="fa fa-list"></i> Listado de registros</h3>
        </div>
        <div class="box-header with-border">
            <div class="row">
                ...
                <div class="col-xs-12"> 
                    <table id="table" class="table table-bordered table-striped table-hover" style="width: 100%">
                        <thead>
                            ...
                            <th>Aprobador</th>
                            <th width="50px" class="no-sort text-center">Acciones</th>
    ...
    <script>
        ...
        jQuery(document).ready(function() {
            ...
            table = $('#table').DataTable({
                "ajax": {
                    "url": "<?php echo site_url('prefactura/get_prefacturas')?>",
                    'type': 'POST',
                    "dataSrc":"",
                    "data": function ( d ) {
                        d.periodo = $("#fecha").val()
                ...
                "columns":[
                {data : "id_prefactura"     ,"className": "text-center"},
                ...
                {data : "fecha_aprobacion"  ,"className": "text-center"},
                {data : "aprobador"         ,"className": "text-left"},
                {data : "id_prefactura"     ,"className": "text-center"}
                ],
                "columnDefs": [
                {
                ...
                "createdRow": function (row,data,index){
                    ...
                    $('td',row).eq(3).append('<span class="label '+estado+' center-block lead">'+data.estado+'</span>');

                    $('td',row).eq(7).empty();
                    $('td',row).eq(7).append('<a data-id="'+data.id_prefactura+'" href="'+data.url+'" class="btn btn-sm btn-primary"><i class="fa fa-search"></i></a>');
                    
                    // $('td',row).eq(8).empty();
                    // var usuario_id = <?php echo $this->session->usuid;?>;
                    // if( usuario_id == data.creador){
